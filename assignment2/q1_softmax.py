...
def softmax(x):
  ...
  x -= tf.expand_dims(x_max, -1)
  
  x_exp = tf.exp(x)
  x_sum = tf.reduce_sum(x_exp, reduction_indices=1)
  
  out = x_exp / tf.expand_dims(x_sum, -1)
  ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
  ...
  y = tf.to_float(y)
  temp = tf.reduce_sum(y * yhat, 1)
  #sess = tf.Session()
  #with sess.as_default():
  #    print temp.eval()
  temp = -tf.log(temp)
  out = tf.reduce_sum(temp)
